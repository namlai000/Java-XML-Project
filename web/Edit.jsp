    ...
    <c:forEach items="${requestScope.article.tblNews.tblImageList}" var="images" varStatus="status">
            ...
            for (var i = 0; i < res.length; i++) {
                if (res[i] != '') {
                    ...
                    } else if (format.contains('</iframe>')) {
                        AddVideo(format.replaceAll('<iframe type="text/html" width="640" height="360" src="', '').replaceAll('" frameborder="0"></iframe>', ''));
                    } else {
                        AddParagraph(format);
                    }
                }
        ...
        function AddVideo(link) {
            ...
            var container = document.createElement("p");
            container.className = "parent";
            container.innerHTML = "<input name='video' type='text' class='form-control' placeholder='Youtube URL' value='" + link + "'/> <button class='delete'>X</button>";
            article.appendChild(container);
